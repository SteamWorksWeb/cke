...
/* ── Inner form (needs to be inside the provider) ── */
function AskClayForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      // Get reCAPTCHA v3 token (invisible — no checkbox shown to user)
      const recaptchaToken = await executeRecaptcha("ask_clay_form");
      ...
      const res = await fetch("/api/ask-clay", {
        method: "POST",
  ...
  return (
    <form onSubmit={handleSubmit} ref={formRef} className="space-y-6" noValidate>
      ...
      <p className="text-center text-xs text-gray-400 pt-1">
        Your info will never be shared or sold. Ever. Protected by reCAPTCHA.
      </p>
    </form>
  );
}

/* ── Page wrapper — provides reCAPTCHA context ── */
